...
| ~~2026-09-11~~ | **DONE 2026-09-23, as a subprocess bridge, not a library import.** ~~Consume `paper-trader` as a library for strategy backtesting on screened baskets.~~ The README's own "neither imports the other" is a deliberate boundary (confirmed: paper-trader also has no pyproject.toml/setup.py to import even if that boundary were reversed), so `scripts/local_basket_backtest.py` calls paper-trader's CLI as a subprocess per top-ranked ticker and parses its printed report instead — same data, same separation, no shared code | 14 |
